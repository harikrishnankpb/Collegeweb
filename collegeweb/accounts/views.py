...
from webmain.models import Teacher,Student,extendedUser
...
def tregister(requests):
    if requests.method=='POST':
        ...
        if password1!=password2:
            ...
            return redirect('tregister')
        try:
            user=User.objects.create_user(username=username,password=password1,first_name=name,last_name='Teacher',email=email)
            newTeacher=Teacher(name=name,phnumber=phnumber,qualification=qualification,user=user)
            user.save()
            newTeacher.save()
            newExtendedUser=extendedUser(user=user,isTeacher=True,isStudent=False)
...
def sregister(requests):
    if requests.method=='POST':
        ...
        if password1!=password2:
            ...
            return redirect('sregister')
        try:
            user=User.objects.create_user(username=username,password=password1,first_name=name,last_name='',email=email)
            user.save()
            newExtendedUser=extendedUser(user=user,isTeacher=False,isStudent=True)
            newExtendedUser.save()
            newStudent=Student(name=name,sem=sem,phnumber=phnumber,dob=dob,user=user)
            newStudent.save()
            auth.logout(requests)
            messages.info(requests,'User registered')
        except:
